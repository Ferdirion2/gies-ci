...
<section x-data="{
    ...
    {{-- HERO --}}
    {{-- Même conteneur horizontal que les autres sections --}}
...
<section class="bg-slate-900 py-8 text-white sm:py-10">
    <div class="mx-auto max-w-6xl px-6">
        ...
        }" class="grid gap-4 grid-cols-3 items-stretch">
...
<section class="bg-white py-24 text-slate-900">
    ...
    <div class="max-w-6xl mx-auto px-6">
        ...
        <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <a href="{{ route('site.services.show', $service) }}" data-index="{{ $loop->index }}" x-data="{ show:false, delay: {{ $loop->index }} * 160 }" x-init="if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) { show = true; } else { setTimeout(() => show = true, 220 + delay); }" :class="show ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-6'" class="group block rounded-[40px] border border-slate-200/80 bg-slate-50 p-8 shadow-[0_20px_80px_-40px_rgba(15,23,42,0.08)] transition-all duration-700 ease-out hover:-translate-y-1 hover:shadow-lg hover:border-orange-500/30 hover:bg-white">
...
<section class="bg-slate-950 py-16 text-white sm:py-20">
    ...
    <div class="max-w-6xl mx-auto px-6">
        ...
        <div class="grid gap-6 lg:grid-cols-3 lg:grid-rows-2">
            ...
            <a href="{{ route('site.realisations.show', $realisation) }}" class="group block overflow-hidden rounded-[44px] bg-slate-950 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.5)] transition duration-300 hover:-translate-y-1">
...
<section class="relative py-20">
    <!-- Background covering the right half of the section on large screens -->
    <div class="hidden lg:block absolute inset-y-0 right-0 w-1/2 bg-brand-blue/6 pointer-events-none"></div>
    <div class="max-w-6xl mx-auto px-6">
        <div x-data="{ show: false }" x-init="if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) { show = true } else { setTimeout(() => show = true, 220) }" class="grid gap-8 lg:grid-cols-2 items-stretch">
...
<section class="relative isolate overflow-hidden bg-slate-950 py-20 text-white sm:py-24" x-data="{ show: false }" x-init="if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) { show = true } else { setTimeout(() => show = true, 180) }">
